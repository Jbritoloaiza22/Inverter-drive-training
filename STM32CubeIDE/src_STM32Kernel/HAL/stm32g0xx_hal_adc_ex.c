...
HAL_StatusTypeDef HAL_ADCEx_Calibration_Start(ADC_HandleTypeDef *hadc) {
  ...
  if (LL_ADC_IsEnabled(hadc->Instance) == 0UL) {
    ...
    for (calibration_index = 0UL; calibration_index < 8UL;
         calibration_index++) {
      ...
      calibration_factor_accumulated +=
          (LL_ADC_GetCalibrationFactor(hadc->Instance) + 1UL);
    }
    ...
    /* Case of ADC clocked at low frequency: Delay required between ADC enable
     * and disable actions */
    if (LL_ADC_GetClock(hadc->Instance) == LL_ADC_CLOCK_ASYNC) {
      adc_clk_async_presc =
          LL_ADC_GetCommonClock(__LL_ADC_COMMON_INSTANCE(hadc->Instance));

      /* Delay applied only when ADC clock frequency is much lower than CPU
         clock frequency (in other cases, code execution time is sufficient to
         ensure necessary delay) */
      if (adc_clk_async_presc >= LL_ADC_CLOCK_ASYNC_DIV16) {
        /* Delay loop initialization and execution */
        /* Delay depends on ADC clock prescaler: Compute ADC clock asynchronous
           prescaler to decimal format (for instance, computation from ADC
           prescaler 64 (register bitfield value 0x9) will give decimal value
           64) */
        delay_cpu_cycles =
            (1UL << ((adc_clk_async_presc >> ADC_CCR_PRESC_Pos) - 3UL));

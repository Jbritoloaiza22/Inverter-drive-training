 ...
 *          - Scope entire ADC (differentiation done for compatibility with some
 * other STM32 series featuring ADC groups regular and injected):
 ...
 *          - For all parameters except 'ClockPrescaler' and 'Resolution': ADC
 * enabled without conversion on going on group regular. If ADC is not in the
...
typedef struct {
  uint32_t
      ClockPrescaler; /*!< Select ADC clock source (synchronous clock derived
                         ...
                         case of synchronous clock mode based on HCLK/1, the
                         configuration must be enabled only if the system clock
                         has a 50% duty clock cycle (APB prescaler configured
                         inside RCC  must be bypassed and PCLK clock must have
                                 50% duty cycle). Refer to reference manual for
                         details. Note: In case of usage of asynchronous clock,
                         the selected clock must be preliminarily enabled at RCC
                         top level. Note: This parameter can be modified only if
  ...
  uint32_t ScanConvMode; /*!< Configure the sequencer of ADC group regular.
                              On this STM32 series, ADC group regular sequencer
                            both modes "fully configurable" or "not fully
                            configurable" are available:
                               - sequencer configured to fully configurable:
                                 sequencer length and each rank affectation to a
                            channel are configurable.
                                  - Sequence length: Set number of ranks in the
                            scan sequence.
                                  - Sequence direction: Unless specified in
                            parameters, sequencer scan direction is forward
                            (from rank 1 to rank n).
                            ...
                            sequence is defined by number of channels set in the
                            sequence, rank of each channel is fixed by channel
                            HW number. (channel 0 fixed on rank 0, channel 1
                            fixed on rank1, ...).
                                  - Sequence direction: Unless specified in
                            parameters, sequencer scan direction is forward
                            (from lowest channel number to highest channel
                            number). This parameter can be associated to
                            parameter 'DiscontinuousConvMode' to have main
                            sequence subdivided in successive parts. Sequencer
                            is automatically enabled if several channels are set
                            (sequencer cannot be disabled, as it can be the case
                              on other STM32 devices):
                              If only 1 channel is set: Conversion is performed
                            in single mode. If several channels are set:
                            Conversions are performed in sequence mode. This
  ...
  FunctionalState
      LowPowerAutoWait; /*!< Select the dynamic low power Auto Delay: new
                           ...
                           (for ADC group regular) has been retrieved by user
                           software, using function HAL_ADC_GetValue(). This
                           feature automatically adapts the frequency of ADC
                           conversions triggers to the speed of the system that
                           reads the data. Moreover, this avoids risk of overrun
                           for low frequency applications. This parameter can be
                           set to ENABLE or DISABLE. Note: It is not recommended
                           to use with interruption or DMA (HAL_ADC_Start_IT(),
                           ...
                           result and trig another conversion start. */

  FunctionalState
      LowPowerAutoPowerOff; /*!< Select the auto-off mode: the ADC automatically
                               powers-off after a conversion and automatically
                               wakes-up when a new conversion is triggered (with
                               startup time between trigger and start of
                               sampling). This feature can be combined with
                               automatic wait mode (parameter
                               'LowPowerAutoWait'). This parameter can be set to
                               ENABLE or DISABLE. */
  ...
  uint32_t
      Overrun; /*!< Select the behavior in case of overrun: data overwritten or
                  preserved (default). This parameter can be a value of @ref
  ...
  uint32_t
      SamplingTimeCommon1; /*!< Set sampling time common to a group of channels.
                                ...
                                Conversion time is the addition of sampling time
                              and processing time (12.5 ADC clock cycles at ADC
                              resolution 12 bits, 10.5 cycles at 10 bits, 8.5
                              cycles at 8 bits, 6.5 cycles at 6 bits). Note: On
                              this STM32 family, two different sampling time
                              settings are available, each channel can use one
                              of these two settings. On some other STM32 devices
                                      this parameter in channel wise and is
                              located into ADC channel initialization structure.
                                This parameter can be a value of @ref
                              ADC_HAL_EC_CHANNEL_SAMPLINGTIME Note: In case of
                              ...
                              be adjusted in function of ADC clock frequency and
                              sampling time setting) Refer to device datasheet
                              for timings values, parameters TS_vrefint,
                                      TS_vbat, TS_temp (values rough order: few
                              tens of microseconds). */

  uint32_t
      SamplingTimeCommon2; /*!< Set sampling time common to a group of channels,
                              second common setting possible. Unit: ADC clock
                              cycles Conversion time is the addition of sampling
                              time and processing time (12.5 ADC clock cycles at
                              ADC resolution 12 bits, 10.5 cycles at 10 bits,
                                  8.5 cycles at 8 bits,
                                  6.5 cycles at 6 bits).
                                Note: On this STM32 family, two different
                              sampling time settings are available, each channel
                              can use one of these two settings. On some other
                              STM32 devices this parameter in channel wise and
                              is located into ADC channel initialization
                              structure. This parameter can be a value of @ref
                              ADC_HAL_EC_CHANNEL_SAMPLINGTIME Note: In case of
                              usage of internal measurement channels
                              (VrefInt/Vbat/TempSensor) sampling time
                              constraints must be respected (sampling time can
  ...
  FunctionalState
      OversamplingMode; /*!< Specify whether the oversampling feature is enabled
                           ...
                           there is no conversion is ongoing on ADC group
                           regular. */
  ...
  uint32_t
      TriggerFrequencyMode; /*!< Set ADC trigger frequency mode.
                                 This parameter can be a value of @ref
                               ADC_HAL_EC_REG_TRIGGER_FREQ. Note: ADC trigger
                               frequency mode must be set to low frequency when
                                       a duration is exceeded before ADC
                               conversion start trigger event (between ADC
                               enable and ADC conversion start trigger event or
                               between two ADC conversion start trigger event).
                                       Duration value: Refer to device
                               datasheet, parameter "tIdle". Note: When ADC
                               trigger frequency mode is set to low frequency,
                                       some rearm cycles are inserted before
                               performing ADC conversion start, inducing a delay
                               of 2 ADC clock cycles. */
 ...
 *          - For all parameters: ADC disabled or enabled without conversion on
 * going on regular group. If ADC is not in the appropriate state to modify some
 * parameters, these parameters setting is bypassed without error reporting (as
 * it can be the expected behavior in case of intended action to update another
...
typedef struct {
  ...
  uint32_t Rank; /*!< Add or remove the channel from ADC regular group sequencer
                    and specify its conversion rank. This parameter is dependent
                    on ScanConvMode:
                      - sequencer configured to fully configurable:
                    ...
                    ...). Despite the channel rank is fixed, this parameter
                    allow an additional possibility: to remove the selected rank
                    (selected channel) from sequencer. This parameter can be a
                    value of @ref ADC_HAL_EC_REG_SEQ_RANKS */
  ...
  uint32_t
      SamplingTime; /*!< Sampling time value to be set for the selected channel.
                       ...
                       12 bits, 10.5 cycles at 10 bits, 8.5 cycles at 8 bits,
                          6.5 cycles at 6 bits).
                         This parameter can be a value of @ref
                       ADC_HAL_EC_SAMPLINGTIME_COMMON Note: On this STM32
                       family, two different sampling time settings are
                       available (refer to parameters "SamplingTimeCommon1" and
                       "SamplingTimeCommon2"), each channel can use one of these
                       two settings.
...
/**
  ...
  *          - For all parameters except 'HighThreshold', 'LowThreshold': ADC
  disabled or ADC enabled without conversion on going on ADC groups regular.
  *          - For parameters 'HighThreshold', 'LowThreshold': ADC enabled with
  conversion on going on regular.
  */
typedef struct {
  uint32_t
  ...
  uint32_t
      WatchdogMode; /*!< Configure the ADC analog watchdog mode: single/all/none
                       ...
                       watchdog mode: single channel or all channels, ADC group
                       regular. For Analog Watchdog 2 and 3: Several channels
                       can be monitored by applying successively the AWD init
                       structure. This parameter can be a value of @ref
...
#define HAL_ADC_STATE_BUSY_INTERNAL                                            \
  (0x00000002UL) /*!< ADC is busy from internal process (ex : calibration,     \
                    ...) */
...
#define HAL_ADC_STATE_INJ_BUSY                                                 \
  (0x00001000UL) /*!< Not available on this STM32 series: A conversion on      \
                    group injected is ongoing or can occur (either by          \
                    auto-injection mode, external trigger, low power auto      \
                    power-on (if feature available), multimode ADC master      \
...
#define HAL_ADC_STATE_INJ_JQOVF                                                \
  (0x00004000UL) /*!< Not available on this STM32 series: Injected queue       \
                    overflow occurrence */
...
#define HAL_ADC_STATE_MULTIMODE_SLAVE                                          \
  (0x00100000UL) /*!< Not available on this STM32 series: ADC in multimode     \
                    slave state, controlled by another ADC master (when        \
                    feature available) */
...
#endif /* USE_HAL_ADC_REGISTER_CALLBACKS */
{
  ...
  uint32_t ADCGroupRegularSequencerRanks; /*!< ADC group regular sequencer
                                             memorization of ranks setting, used
                                             in mode "fully configurable" (refer
                                             to parameter 'ScanConvMode') */
#if (USE_HAL_ADC_REGISTER_CALLBACKS == 1)
...
#define ADC_SCAN_DIRECTION_FORWARD                                             \
  (ADC_SCAN_SEQ_FIXED) /* For compatibility with other STM32 series */
#define ADC_SCAN_DIRECTION_BACKWARD                                            \
  (ADC_SCAN_SEQ_FIXED_BACKWARD) /* For compatibility with other STM32 series   \
                                 */
/**
 * @}
 */
...
/** @defgroup ADC_HAL_EC_SAMPLINGTIME_COMMON  ADC instance - Sampling time
...
#define ADC_RIGHTBITSHIFT_1                                                    \
  (LL_ADC_OVS_SHIFT_RIGHT_1) /*!< ADC oversampling right shift of 1 rank */
#define ADC_RIGHTBITSHIFT_2                                                    \
  (LL_ADC_OVS_SHIFT_RIGHT_2) /*!< ADC oversampling right shift of 2 rank */
#define ADC_RIGHTBITSHIFT_3                                                    \
  (LL_ADC_OVS_SHIFT_RIGHT_3) /*!< ADC oversampling right shift of 3 rank */
#define ADC_RIGHTBITSHIFT_4                                                    \
  (LL_ADC_OVS_SHIFT_RIGHT_4) /*!< ADC oversampling right shift of 4 rank */
#define ADC_RIGHTBITSHIFT_5                                                    \
  (LL_ADC_OVS_SHIFT_RIGHT_5) /*!< ADC oversampling right shift of 5 rank */
#define ADC_RIGHTBITSHIFT_6                                                    \
  (LL_ADC_OVS_SHIFT_RIGHT_6) /*!< ADC oversampling right shift of 6 rank */
#define ADC_RIGHTBITSHIFT_7                                                    \
  (LL_ADC_OVS_SHIFT_RIGHT_7) /*!< ADC oversampling right shift of 7 rank */
#define ADC_RIGHTBITSHIFT_8                                                    \
  (LL_ADC_OVS_SHIFT_RIGHT_8) /*!< ADC oversampling right shift of 8 rank */
/**
 * @}
 */
...
/** @defgroup ADC_HAL_EC_REG_TRIGGER_FREQ  ADC group regular - Trigger frequency
 * mode
 * @note ADC trigger frequency mode must be set to low frequency when a duration
 * is exceeded before ADC conversion start trigger event (between ADC enable and
 * ADC conversion start trigger event or between two ADC conversion start
...
#define ADC_TRIGGER_FREQ_LOW                                                   \
  (LL_ADC_TRIGGER_FREQ_LOW) /*!< Trigger frequency mode set to low frequency.  \
                             */
/**
 * @}
 */
...
#define ADC_IT_CCRDY                                                           \
  ADC_IER_CCRDYIE /*!< ADC channel configuration ready interrupt source */
#define ADC_IT_EOSMP                                                           \
...
#define ADC_FLAG_CCRDY                                                         \
  ADC_ISR_CCRDY /*!< ADC channel configuration ready flag */
...
#define ADC_CFGR1_OVERRUN(_OVERRUN_MODE_)                                      \
  (((_OVERRUN_MODE_) != (ADC_OVR_DATA_PRESERVED)) ? (ADC_CFGR1_OVRMOD)         \
                                                  : (0x00000000UL))
 ...
 *        Thresholds have to be left-aligned on bit 11, the LSB (right bits) are
...
#define ADC_AWD1THRESHOLD_SHIFT_RESOLUTION(__HANDLE__, _Threshold_)            \
  ((_Threshold_) << ((((__HANDLE__)->Instance->CFGR1 & ADC_CFGR1_RES) >> 3U) * \
                     2U))
...
#define IS_ADC_CHANNEL(CHANNEL)                                                \
  (((CHANNEL) == ADC_CHANNEL_0) || ((CHANNEL) == ADC_CHANNEL_1) ||             \
   ...
   ((CHANNEL) == ADC_CHANNEL_VREFINT) || ((CHANNEL) == ADC_CHANNEL_VBAT))
...
#define IS_ADC_EVENT_TYPE(EVENT)                                               \
  (((EVENT) == ADC_EOSMP_EVENT) || ((EVENT) == ADC_AWD1_EVENT) ||              \
   ((EVENT) == ADC_AWD2_EVENT) || ((EVENT) == ADC_AWD3_EVENT) ||               \
   ((EVENT) == ADC_OVR_EVENT))
...
/** @defgroup ADC_regular_nb_conv_verification ADC Regular Conversion Number
 * Verification
 * @{
 */
...
#define ADC_FLAG_POSTCONV_ALL                                                  \
  (ADC_FLAG_AWD | ADC_FLAG_OVR | ADC_FLAG_EOS | ADC_FLAG_EOC)
 ...
 *            @arg @ref ADC_IT_CCRDY  ADC channel configuration ready interrupt
 * source
...
/**
  ...
  *                                        On STM32G0, refer to device datasheet
  parameter "Avg_Slope".
